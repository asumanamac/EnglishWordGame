    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        btnRegister.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                ...
                if (!name.isEmpty() && !email.isEmpty() && !password.isEmpty() && error==false) {
                    registerUser(name, email, password);
                } else {
                    error=true;
                    errorMessage+="Please check crediantials\n";
                }

                if(!password.matches(confirm_password)){
                    error = true;
                    errorMessage = "Passwords are differrent!!!";
                }

                if( error){//hata varsa AlertDialog ile kullanıcıyı uyarıyoruz.
                    android.support.v7.app.AlertDialog alertDialog = new android.support.v7.app.AlertDialog.Builder(RegisterActivity.this).create();
